...
def get_yearly_sales():
    ...
    product_sales_present = True
    if 'product_sales' in request.args:
        product_sales = (request.args['product_sales'])

    else:
        product_sales_present = False
    ...
    if product_sales_present == True:
        top_selling = bbazar['product_description'].value_counts(ascending=False).head(10).to_dict()
        low_selling = bbazar['product_description'].value_counts(ascending=True).head(10).to_dict()
...
def get_quarterly_sales():
    ...
    if product_sales_present == True:
        top_selling = bbazar2[['quarter','product_description'].value_counts(ascending=False).head(10).to_dict()
        low_selling = bbazar2['product_description'].value_counts(ascending=True).head(10).to_dict()
        return jsonify(top_selling, low_selling)
...
@application.route('/api/v1.0/promo_sales/', methods=['GET'])
def get_promo_sales():
